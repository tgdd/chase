...
module Signature : sig
    ...
    (** Add the predicate with the arity to the signature. If the pred is
    ...
    (** Converts the sig to a human readable string *)
    val show : sig_t -> string
    (** Determines if the two signatures are the same *)
    val equal : sig_t -> sig_t -> bool
end
type sig_t = Signature.sig_t

(** Determines the free variable occurances present in a formula. *)
